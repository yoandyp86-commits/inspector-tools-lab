...
class App extends React.Component {
  constructor(props) {
    ...
    this.onExportSavedQueries = this.onExportSavedQueries.bind(this);
    this.onImportSavedQueries = this.onImportSavedQueries.bind(this);
  ...
  onExportSavedQueries(e) {
    e.preventDefault();
    let {model} = this.props;
    const queries = model.savedHistory.list || [];
    if (queries.length === 0) {
      alert("There are no saved queries to export.");
      model.toggleSavedOptions();
      model.didUpdate();
      return;
    }
    const payload = {
      type: "sfir-saved-queries",
      version: 1,
    ...
    a.href = url;
    a.download = "saved-queries-" + stamp + ".json";
      ...
      reader.onload = () => {
        try {
          ...
          if (!Array.isArray(queries)) {
            throw new Error("The file does not contain a list of saved queries.");
          }
          let imported = 0;
          queries.forEach(q => {
            if (q && typeof q.query === "string") {
              model.savedHistory.add({query: q.query, useToolingApi: !!q.useToolingApi});
              imported++;
            }
          });
          alert("Imported " + imported + " saved quer" + (imported === 1 ? "y" : "ies") + ".");
          model.didUpdate();
        } catch (err) {
          alert("Could not import saved queries: " + err.message);
        }
      };
              ...
              h("div", {className: "query-history-controls"},
                ...
                h("div", {className: "slds-dropdown-trigger slds-dropdown-trigger_click " + (model.expandSavedOptions ? "slds-is-open" : "slds-is-closed")},
                  h("div", {className: "slds-dropdown slds-dropdown_right"},
                    ...
                    h("div", {className: "slds-dropdown__item"},
                      h("a", {href: "#", onClick: this.onExportSavedQueries, title: "Export saved queries to a file"}, "Export Saved Queries")
                    ),
                    h("div", {className: "slds-dropdown__item"},
                      h("a", {href: "#", onClick: this.onImportSavedQueries, title: "Import saved queries from a file"}, "Import Saved Queries")
                    )
                  )
                ),
              ),
